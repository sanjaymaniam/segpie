...
def predict(image, training):
    ...
    conv1a = conv(img, training, D=3, K=64, F=3, S=1, scope = "conv1a")
    conv1b = conv(conv1a, training, D=64, K=64, F=3, S=1, scope = "conv1b")
    pool1, pool1_indices = max_pool(conv1b, F=2, S=2, scope = "pool1")

    conv2a = conv(pool1, training, D=64, K=64, F=3, S=1, scope = "conv2a")
    conv2b = conv(conv2a, training, D=64, K=64, F=3, S=1, scope = "conv2b")
    pool2, pool2_indices = max_pool(conv2b, F=2, S=2, scope = "pool2")

    conv3a = conv(pool2, training, D=64, K=64, F=3, S=1, scope = "conv3a")
    conv3b = conv(conv3a, training, D=64, K=64, F=3, S=1, scope = "conv3b")
    pool3, pool3_indices = max_pool(conv3b, F=2, S=2, scope = "pool3")

    conv4a = conv(pool3, training, D=64, K=64, F=3, S=1, scope = "conv4a")
    conv4b = conv(conv4a, training, D=64, K=64, F=3, S=1, scope = "conv4b")
    pool4, pool4_indices = max_pool(conv4b, F=2, S=2, scope = "pool4")

    unpool4 = unpool(pool4, conv4b.get_shape(),64,64,2, S=2, scope="unpool4")
    # unpool4 = max_indices_unpool(pool4, pool4_indices, S=2, scope = "unpool4")
    deconv4b = conv(unpool4, training, D = 64, K=64, F=3, S=1, scope = "deconv4b")
    deconv4a = conv(deconv4b, training, D = 64, K=64, F=3, S=1, scope = "deconv4a")

    unpool3 = unpool(deconv4a, conv3b.get_shape(),64,64,2, S=2, scope="unpool3")
    # unpool3 = max_indices_unpool(deconv4, pool3_indices, S=2, scope = "unpool3")
    deconv3b = conv(unpool3, training, D=64, K=64, F=3, S=1, scope = "deconv3b")
    deconv3a = conv(deconv3b, training, D=64, K=64, F=3, S=1, scope = "deconv3a")

    unpool2 = unpool(deconv3a, conv2b.get_shape(),64,64,2, S=2, scope="unpool2")
    # unpool2 = max_indices_unpool(deconv3, pool2_indices, S=2, scope = "unpool2")
    deconv2b = conv(unpool2, training,D=64, K=64, F=3, S=1, scope = "deconv2b")
    deconv2a = conv(deconv2b, training,D=64, K=64, F=3, S=1, scope = "deconv2a")

    unpool1 = unpool(deconv2a, conv1b.get_shape(),64,64,2, S=2, scope="unpool1")
    # unpool1 = max_indices_unpool(deconv2, pool1_indices, S=2, scope = "unpool1")
    deconv1b = conv(unpool1, training,D=64, K=64, F=3, S=1, scope = "deconv1b")
    deconv1a = conv(deconv1b, training,D=64, K=64, F=3, S=1, scope = "deconv1a")

    logits = conv(deconv1a, training, D=64, K=11, F=1, S=1, scope="final_logits")
